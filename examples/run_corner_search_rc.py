...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run corner search on RC lowpass.")
    parser.add_argument(
        "--netlist",
        type=Path,
    ...
    strategy = CornerSearchStrategy()
    if resolve_ngspice_executable(strategy.sim_run_op.ngspice_bin) is None:
        print("ngspice not found; skipping run_corner_search_rc")
        return
