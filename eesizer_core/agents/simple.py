...
class SimpleSizingAgent(Agent):
    ...
    def build_plan(self, context: ExecutionContext) -> Sequence[Message]:
        ...
        return [
            Message(role=MessageRole.SYSTEM, content=plan_note, name=self.metadata.name),
            Message(role=MessageRole.USER, content=target_note, name="target_extraction"),
        ]
    ...
    def run_simulation(self, context: ExecutionContext) -> MutableMapping[str, float]:
        ...
        if metrics:
            ...
            raw_metrics = self.simulator.run(netlist_text, None, context.working_dir)
    ...
    def _tool_run_simulation(
        ...
        raw_metrics = self.simulator.run(netlist_text, deck, context.working_dir)
        metrics = aggregate_measurement_values(raw_metrics)
        metrics = self._merge_with_cached_metrics(context, metrics)
