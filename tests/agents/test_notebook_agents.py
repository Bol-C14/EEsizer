...
def _build_agent(agent_cls, simulator, output_root: Path, recordings: dict | None = None):
...
def test_agents_emit_notebook_prompts(
    agent_cls, mini_netlist, rich_mock_simulator, tmp_path, recorded_llm_responses
):
    agent = _build_agent(
        agent_cls, rich_mock_simulator, tmp_path / "outputs", recordings=recorded_llm_responses
    )
...
@pytest.mark.parametrize("agent_cls", AGENT_CLASSES)
def test_agents_run_with_rich_metrics(
    agent_cls, mini_netlist, rich_mock_simulator, tmp_path, recorded_llm_responses
):
    agent = _build_agent(
        agent_cls, rich_mock_simulator, tmp_path / "outputs", recordings=recorded_llm_responses
