...
def test_greedy_coordinate_policy_improves_ac_mag(tmp_path):
    ...
    policy = GreedyCoordinatePolicy(init_step=0.2, min_step=0.01, max_step=0.5, max_trials_per_param=6)
    strategy = PatchLoopStrategy(policy=policy)
    cfg = StrategyConfig(
        budget=OptimizationBudget(max_iterations=30, no_improve_patience=10),
