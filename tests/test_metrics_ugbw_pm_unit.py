...
def test_ugbw_pm_one_pole(tmp_path: Path) -> None:
    a0 = 1e3
    fp = 100.0
    ...
    ugbw = metrics.values["ugbw_hz"].value
    pm = metrics.values["phase_margin_deg"].value

    assert ugbw is not None
    assert pm is not None
    assert ugbw == pytest.approx(a0 * fp, rel=0.05)
    assert pm == pytest.approx(90.0, abs=5.0)
