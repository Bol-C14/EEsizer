...
- Mitigation: parsing now returns typed structures (`TaskQuestions`, `TargetValues`) and flows carry state through contexts; no globals are written during LLM parsing.
...
- Mitigation: added `validate_tool_chain` to enforce allowed tool names and ordering (analysis after run_ngspice) and integrated it into both agent entry and `ToolChainRunner`.
...
- Mitigation: all simulations/optimizations now require an explicit per-run `output_dir`; `run_agent` creates a unique run directory, and reporting/copy operations write within that run scope (no shared defaults).
...
- Mitigation: added structured logging across modules (`logging_utils.get_logger`) and replaced prints with leveled logs; LLM calls now raise on errors, optimization loop logs and surfaces failures, and ngspice/analysis paths raise on failure instead of silently continuing.
...
- Mitigation: `offset` now validates data availability and raises on missing points; target parsing keeps THD/DC targets as provided, sets pass flags only when targets are absent, and adds DC gain handling.
...
- Mitigation: all simulation helpers now require an explicit per-run `output_dir`; `run_agent` and runners pass unique run directories so files no longer collide or depend on ambient state.
...
- Mitigation: simulation utilities now assert file existence/size before parsing and raise clear errors when data is missing; paths are constructed from the caller-provided `output_dir`.
